# Git 學習紀錄 - NonaWin 專案

這份文件記錄了在開發 NonaWin 專案過程中所使用的 Git 指令與核心概念。

## 1. 初始設定 (初始化倉庫)

想要將現有的專案開始進行版本控制時，我們使用以下指令：

### 使用指令：
```bash
# 在當前目錄初始化一個新的 Git 倉庫
git init

# 建立 .gitignore 檔案 (這對 .NET 專案至關重要，用來忽略那些編譯產生的暫存檔)
# (我們是手動建立此檔案，但它的作用是告訴 git 哪些檔案「不要」追蹤)

# 將所有檔案加入到「暫存區」(Staging Area)，準備進行第一次提交
git add .

# 建立初始提交 (Commit)
git commit -m "Initial commit: NonaWin v1.0 with Explorer UI and Duplicate Analysis"
```

### 核心概念：暫存區 (The Staging Area)
- **`git init`**：建立一個隱藏的 `.git` 資料夾來追蹤所有變更。
- **`git add`**：將工作目錄中的變更移動到「暫存區」（準備好被儲存的狀態）。
- **`git commit`**：將「暫存區」的狀態拍一張快照，並永久儲存到歷史紀錄中。

---

## 2. 功能開發流程 (分支操作)

**問題**：*我應該直接修改程式碼，還是該開一個分支？*

**最佳實踐**：請務必使用 **分支 (Branches)**。
即使您是獨立開發，使用分支也能幫助您組織思緒，讓您可以隨時「切換情境」或在其上做實驗，失敗了也能輕鬆丟棄。

### 建議的工作流程：
1.  為該功能 **建立一個新分支**：
    ```bash
    git checkout -b feature/您的功能名稱
    ```
2.  **進行修改** 並測試。
3.  **提交變更** 到該分支。
4.  完成後，**合併 (Merge)** 回主分支。

### 常用分支指令：
```bash
# 列出所有分支 (目前所在的分支前會有 * 號)
git branch

# 建立並切換到一個新分支
git checkout -b feature/new-ui-color
# 新版 git (2.23+) 也可以用: git switch -c feature/new-ui-color

# 切換回主分支
git checkout main

# 將功能分支合併進當前分支
git merge feature/new-ui-color
```

## 3. 實戰練習記錄

### 步驟 1：建立分支 (已完成)
您已經成功執行了：
```bash
git checkout -b feature/add-readme
```
現在您位於 `feature/add-readme` 分支上，可以開始進行修改了。

### 步驟 2：進行修改 (下一步)
既然分支名稱叫 `add-readme`，我們來新增一個 `README.md` 說明文件看看。
1. 在專案目錄下建立一個新檔案 `README.md`。
2. 輸入一些專案介紹內容並存檔。

### 步驟 3：提交變更 (待執行)
修改完成後，我們將使用以下指令由 Git 儲存變更：
```bash
git add README.md
git commit -m "Add project documentation"
```

### 💡 重要觀念：為什麼我切換分支了，檔案還在？
**問題**：*我剛剛新增了 README.md，但切回 main 分支時，為什麼檔案沒有消失？*

**原因**：**因為檔案還沒被提交 (Committed)！**
在 Git 中，如果您只是新增了檔案，但還沒執行 `git commit`，這個檔案就像是「浮動」的一樣。Git 會好心地把它保留在工作目錄中，以免您遺失資料。

**解決方法**：
必須在分支中執行 `git add` 和 `git commit`，將檔案「鎖定」在該分支的歷史紀錄中。一旦提交後，再切換到其他分支，Git 知道該檔案不屬於那裡，就會自動幫您隱藏起來。

### 🎓 進階觀念：為什麼不是「切換就自動儲存」？

這是一個初學者最常見的疑問。您可以這樣想像：

1.  **工作目錄 (你的書桌)**：
    您在寫的 `README.md` 就像是書桌上的一張紙。
2.  **分支 (不同的房間)**：
    `main` 和 `feature` 就像是兩個不同的房間。

**Git 的邏輯是：**
當您在 A 房間的書桌上寫了一張紙 (新增檔案)，但還沒放進資料夾歸檔 (Commit)。
這時如果您決定走到 B 房間 (切換分支)，**您會順手把這張紙也帶過去繼續寫**。

Git 怕您是「忘記切換分支就開始工作」，所以它好心地保留了這張紙，讓您帶到正確的房間去歸檔。

**只有當您執行 `git commit` (歸檔)**：
這張紙才會被鎖進 A 房間的櫃子裡。這時候您兩手空空走到 B 房間，桌上就會是乾淨的（或者有 B 房間自己的檔案）。

### 💡 補充：那「暫存 (Add)」的檔案呢？
**問題**：*我已經執行了 `git add` (暫存)，為什麼切換分支時它還是跟著我？*

**答案**：**暫存區 (Staging Area) 也是跟著人的！**
`git add` 只是把紙張整理好放在手邊（準備歸檔），但還沒真正放入櫃子（Commit）。
所以，**即使加入了暫存區，只要沒 Commit，它就會一直跟著您切換分支。**

---

### 步驟 4：切換分支 (重要指令)

**情境**：如果您想暫時回到主程式看看，或者想切換回分支繼續工作。

1.  **切換回主分支 (main)**：
    ```bash
    git checkout main
    ```
    *注意：此時您剛剛新增的 README.md 可能會消失，因為它只存在於 feature/add-readme 分支中，這是正常的！*

2.  **切換回功能分支**：
    ```bash
    git checkout feature/add-readme
    ```
    *神奇的事發生了：README.md 又出現了！*

### 步驟 5：合併分支 (任務完成)

**情境**：您的功能開發完成且測試沒問題了，現在要把成果合併回主程式。

1.  **先回到主分支**：
    (因為我們要「把東西拿進來」，所以要先回到目的地)
    ```bash
    git checkout main
    ```

2.  **執行合併**：
    ```bash
    git merge feature/add-readme
    ```
    這時候，`main` 分支裡也會出現 `README.md` 了！您可以執行 `git log` 查看歷史紀錄，會發現剛剛的提交也出現在這裡了。

### 步驟 6：刪除分支 (保持整潔)

**情境**：既然功能已經合併進來，原本的分支就不需要了，留著只會混淆視聽。

1.  **刪除已合併的分支**：
    (參數 `-d` 代表 delete)
    ```bash
    git branch -d feature/add-readme
    ```
    *Git 很聰明，如果這個分支還有東西沒合併，它會阻止您刪除（這時要用 `-D` 強制刪除）。*

## 4. 常見工作情境Ｑ＆Ａ

## 4. 常見工作情境Ｑ＆Ａ

### Q1：個人開發流程是這樣嗎？
**問題**：*如果不只我一個人開發，我就是一直 Commit 分支，直到要併到 Main 對嗎？*

**答案**：**完全正確！💯**
這就是最標準的「Feature Branch Workflow (功能分支工作流)」。
1.  想要加功能 A → 開 `feature/A` 分支
2.  一直改、一直 Commit (都在 `feature/A` 裡)
3.  功能 A 寫完了、測好了
4.  切回 `main` 把 `feature/A` 併進來

這樣可以確保 `main` 永遠是乾淨、穩定、隨時可以發布的版本。不會因為您正在寫一個開發到一半的功能，導致整個程式跑不動。

### Q2：多人開發會不會衝突？
**問題**：*如果我 Check 了一個分支，同事也由 Main Check 了一個分支，這樣是不是不會衝到？*

**答案**：**是的，開發當下完全不會衝到！**
您可以把它想像成：
1.  **Main** 是公用佈告欄。
2.  **您開分支** = 您影印了一份佈告欄的內容回您的位子上改。
3.  **同事開分支** = 同事也影印了一份回他的位子上改。

在你們「各自改各自的影印本」時，完全互不干擾。

**這時會發生什麼事？**
只有在最後「貼回佈告欄 (Merge)」的時候，才有可能遇到狀況：
-   如果您改的是「登入頁面」，同事改的是「關於我們」。
    👉 **完美合併**：Git 會聰明地把兩邊的修改都貼上去，沒事。
-   如果您改了 `Form1.cs` 的第 10 行，同事剛好也改了 `Form1.cs` 的第 10 行。
    👉 **發生衝突 (Conflict)**：這時候 Git 會停下來問您：「ㄟ，這一行你們兩個都要改，我要聽誰的？」這就是所謂的「解衝突」，通常是後合併的人要處理。

### Q3：我要怎麼知道現在在哪個分支？
**問題**：*我要怎麼知道，目前是在 main 還是 branch？*

**答案**：有兩個簡單的指令！

1.  **看列表 (最直覺)**：
    ```bash
    git branch
    ```
    👉 **找星星 (`*`)**：列表前有 `*` 號且變顏色的，就是您目前所在的分支。
    例如：
    ```
      feature/add-readme
    * main              <-- 您在這裡！
    ```

2.  **看狀態 (資訊最細)**：
    ```bash
    git status
    ```
    第一行就會寫：`On branch main` (位於 main 分支)。

## 5. 總結：標準個人開發流程 (Standard Workflow)

這就是您剛剛完成的完整循環，也是全球工程師每天在做的事。即使是一個人開發，這樣做也能保證程式碼整潔、安全。

### 步驟回顧 (以剛剛的「自動更新功能」為例)：

1.  **開新分支 (拿到新任務時)**：
    ```bash
    git checkout -b feature/refresh-status
    ```

2.  **開發與提交 (存檔)**：
    (修改程式碼後...)
    ```bash
    git add .
    git commit -m "Implement auto-refresh directory status"
    ```

3.  **切回主線 (準備合併)**：
    ```bash
    git checkout main
    ```

4.  **合併分支 (驗收成果)**：
    ```bash
    git merge feature/refresh-status
    ```

5.  **刪除分支 (打掃乾淨)**：
    ```bash
    git branch -d feature/refresh-status
    ```

---
**💡 為什麼要這麼麻煩？**
因為這樣做，您的 `main` 分支永遠都是「隨時可以用的版本」。如果新功能寫爛了，只要把分支刪掉就好，`main` 完全不受影響。這就是版本控制的精髓！

### Q4：為什麼不能直接在 Main 改，改壞了再還原就好？
**問題**：*我以前的想法是，反正改壞了只要回到上一個 Main 的狀態就好，為什麼還需要分支？*

**答案**：這是一個很棒的問題！差別在於「過程的代價」。

想像您正在 Main 上面蓋一棟新房子（開發新功能）：
1.  **直接在 Main 改 (單線作業)**：
    -   您把地基挖開了，房子現在是「不能住」的狀態。
    -   突然老闆說：「舊的窗戶破了，現在馬上要修！」
    -   **慘了**：您必須先把挖開的地基填回去（還原），才能去修窗戶。修完窗戶，又要重新挖地基。
    -   **缺點**：Main 會有一段時間是「壞掉的」，而且要切換工作很麻煩。

2.  **開分支改 (平行時空)**：
    -   您複製了一個 Main 到「工地分支」去挖地基。
    -   Main 依然是那棟「完好可住」的房子。
    -   老闆說要修窗戶？沒問題！切回 Main，馬上修好，完全不影響工地那邊的進度。
    -   **優點**：Main 永遠是好的，您可以隨時暫停手邊工作去處理急事。

**結論**：
分支給了您「**隨時可以暫停、隨時可以後悔、且不影響主程式**」的超能力。直接在 Main 改，您就失去了這些彈性。

### Q5：Main 變了，合併回我的分支會有問題嗎？
**問題**：*如果在修浴室（更新 Main）的期間，我的功能分支（客廳施工）也還在改。之後合併時，那個修浴室的變更還會保留嗎？會不會有問題？*

**答案**：**請放心，通常不會有問題，而且修好的浴室會保留下來！**

這是 Git 最聰明的地方，它會進行 **三方合併 (3-way merge)**：
1.  **Main**：有修好的浴室。
2.  **Feature**：有新蓋的客廳。
3.  **結果**：**新客廳 ＋ 修好的浴室** (兩者兼得！)。

**什麼時候才會有問題？**
只有當「修浴室」跟「蓋客廳」剛好都動到**同一面牆壁 (同一行程式碼)** 時，Git 才會舉手說：「這裡我不知道要聽誰的（衝突），請人類來決定。」
除此之外，Git 都能完美自動合併。

### Q6：學習紀錄 (git_learning_log.md) 現在屬於哪個分支？
**問題**：*這份文件現在是不是還不屬於任何分支？所以我在創建分支時，它也會跟著我對嗎？*

**答案**：**一半對，一半錯！**

1.  **檔案本身**：其實已經屬於 `main` 囉！（因為我們在前幾次提交時已經把它存進去了）。
2.  **最新的修改 (Q3~Q6)**：這些「新增的文字」目前還沒存檔 (Commit)！

**狀態：Modified (已修改)**
當您輸入 `git status` 時，會看到它在 `Changes not staged for commit` (紅色 modified)。

**結論**：
即便檔案已經歸檔過，只要**有新的修改還沒 Commit**，這些「新的修改」就會像浮動的紙條一樣，跟著您切換分支！直到您再次執行 `git add` + `git commit` 為止。

### Q7：指令中的 -b 是什麼意思？
**問題**：*`git checkout -b` 的 `b` 是指新的分支 (New Branch) 對嗎？*

**答案**：**是的，完全正確！**

-   **`-b`** = **Create Branch (建立分支)**。

**比較一下差異：**
1.  **`git checkout feature/refresh-status`** (沒有 -b)
    👉 意思：我要 **切換** 到一個 **已經存在** 的分支。
    *(如果分支不存在，Git 會報錯說找不到)*

2.  **`git checkout -b feature/new-feature`** (有 -b)
    👉 意思：我要 **建立** 一個新分支，**並且立刻切換** 過去。

**小口訣**：
-   要去**舊**房間：直接 `checkout`
-   蓋個**新**房間進去：要加 `-b`

### Q8：為什麼 `git branch -b` 會失敗？
**問題**：*我輸入 `git branch -b feature/我的功能`，但是出現錯誤，是因為要用大寫 B 嗎？*

**答案**：**不是喔，是因為指令用錯了！**

這也是初學者超容易搞混的地方：

| 指令 | 說明 | 參數 |
| :--- | :--- | :--- |
| **`git checkout`** | 切換分支 | 可以用 **`-b`** (代表順便**建**一個新的) |
| **`git branch`** | 只有管理分支 (列表/刪除) | **不能**用 `-b` |

**正確寫法**：
-   ❌ `git branch -b feature/我的功能` (Git 會說：我不懂 -b 是什麼)
-   ✅ `git checkout -b feature/我的功能` (正確！建立並切換)

**小撇步**：
只要記得「我要**換**到新分支去做事」，所以動作是 **Checkout (切換)**，只是順便加個 **-b (Branch/建立)** 而已。

### Q9：在分支開發時，要什麼時候 Commit？
**問題**：*我切到分支後，是一直寫程式，中間需要做暫存嗎？還是全部寫完再一次 Commit？*

**答案**：**建議：少量多餐，分段存檔！**

您可以把 Commit 想像成玩遊戲的 **「存檔點 (Save Point)」**。

-   **一次全寫完再 Commit**：
    像是玩了 3 小時都沒存檔，如果不小心打輸王（程式改壞了），您只能**全部重來**。

-   **分段 Commit (推薦)**：
    像是打完一個小怪就存檔一次。
    1.  改完 UI 介面 → `git add` + `git commit -m "Update UI"`
    2.  寫完邏輯 → `git add` + `git commit -m "Add logic"`
    3.  修好 Bug → `git add` + `git commit -m "Fix bug"`

**好處**：
如果您在寫「邏輯」的時候寫壞了，您可以輕鬆回到「改完 UI」的那個存檔點，而不需要整個重做。

**結論**：
不必等到全部做完！只要完成一個「小階段」或者覺得「現在這個狀態不錯，想保留」，就可以執行 `add` + `commit`。在分支裡，您想 Commit 幾次都可以！

### Q10：功能還沒做完（例如只加了按鈕）也可以 Commit 嗎？
**問題**：*所以不一定要全部做好、做完？簡單的一個加上按鈕，也可以 Commit 嗎？*

**答案**：**是的！這才是最高級的做法！👍**
這叫做 **「原子提交 (Atomic Commit)」**。

**為什麼這樣比較好？**
-   **Debug 容易**：如果程式壞了，您可以很清楚知道是「加了按鈕」壞的，還是「寫了邏輯」壞的。如果全部混在一起 Commit，要找兇手就跟大海撈針一樣。
-   **寫起來輕鬆**：心理壓力比較小，完成一點點就存檔，很有成就感。

**唯一的小原則**：
雖然可以切很細，但建議每個 Commit 至少要是 **「可執行的」** (編譯會過)。
也就是說，不要程式碼寫一半斷在奇怪的地方就 Commit，除此之外，切得越細越好！

### Q11：我沒 Push 前，同事真的都看不到嗎？
**問題**：*如果我做了暫存 (Add)、也做了提交 (Commit)，這些是不是都只在我本機？只要沒推到 Remote，同事都不會知道？*

**答案**：**完全正確！這就是 Git 最讓人安心的地方。** 🔒

在 Git 的世界裡，有明顯的界線：
*   **本機 (Local)**：不管您 Add 了幾次、Commit 了幾百個版本，只要沒有執行 `git push`，這一切 **全部都只有您自己看得到**。
*   **遠端 (Remote/GitHub)**：只有執行 `git push` 後，這些 Commit 才會被上傳，同事才看得到。

**好處**：您可以盡情在本地嘗試大量實驗性的改動，把 Commit 紀錄弄得很亂也沒關係（之後還可以整理），等到確定完美了再 Push 出去給大家看。

### Q12：Git 和 SVN 有什麼不同？
**問題**：*Git 很適合本地個人使用對嗎？這點和 SVN 不同？*

**答案**：**您切中要點了！這就是兩者最大的差別。**

| 特性 | Git (分散式) | SVN (集中式) |
| :--- | :--- | :--- |
| **斷網能存檔嗎？** | **可以！** (Commit 是存本機) | **不行！** (Commit 是直接傳伺服器) |
| **個人使用** | **超適合** (自己玩也很開心) | 不適合 (一定要架個 Server) |
| **分支成本** | **極低** (開分支跟喝水一樣快) | 很高 (複製整個目錄，很重) |

**總結**：
SVN 就像是大家**共用同一台主機**打報告，沒網路就不能存檔。
Git 就像是**每個人都有一台自己的電腦**，您可以隨時隨地存檔 (Commit)，最後有網路時再上傳 (Push) 整合即可。所以即便是一個人開發，Git 也是最強大的時光機工具！

